...
describe("describeScatterConfidence", () => {
  ...
  it("treats the structure-captured threshold as exclusive on the low side", () => {
    //: The boundary itself reads as real, since the threshold is strictly below.
  ...
  //: An older payload keeps its AMI reading, so only the layout claim is dropped.
  it("drops the layout claim, keeping the AMI/ARI reading, when the share is absent", () => {
    const result = describeScatterConfidence(undefined, 0.22, 0.27);
    expect(result.level).toBe("moderate");
    expect(result.message).not.toContain("NaN");
    expect(result.message).toContain("0.22");
    expect(result.message).toContain("0.27");
  });

  it("still reads as low on an indistinguishable partition when the share is absent", () => {
    const result = describeScatterConfidence(undefined, 0.02, -0.004);
    expect(result.level).toBe("low");
    expect(result.message).not.toContain("NaN");
    expect(result.message).toContain("0.02");
  });

  it("treats a non-finite share the same as an absent one", () => {
    for (const share of [Number.NaN, null]) {
      expect(describeScatterConfidence(share, 0.22, 0.27).message).not.toContain("NaN");
    }
  });

  it("never prints a percentage it does not have", () => {
    expect(describeScatterConfidence(undefined, 0.22, 0.27).message).not.toMatch(/\d+%/);
  });
});
