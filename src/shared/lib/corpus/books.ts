...
export function bookOfPsalm(psalmNumber: number): PsalmBook {
  const book = BOOKS.find((b) => psalmNumber >= b.range[0] && psalmNumber <= b.range[1]);
  if (!book) {
    throw new RangeError(`Psalm number out of range (1-150): ${String(psalmNumber)}`);
  }
  return book;
}
...
/** The last psalm of every book but the last, as grid divider positions. */
