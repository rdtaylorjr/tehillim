...
describe("sortRows", () => {
  it("sorts numeric rows ascending", () => {
    ...
    expect(sortRows(rows, "auc", "asc").map((r) => r.auc)).toEqual([0.3, 0.5, 0.7]);
  });

  it("sorts numeric rows descending", () => {
    const rows = [{ auc: 0.7 }, { auc: 0.3 }, { auc: 0.5 }];
    expect(sortRows(rows, "auc", "desc").map((r) => r.auc)).toEqual([0.7, 0.5, 0.3]);
  });
  ...
  it("treats a NaN or missing numeric value as -Infinity, sorting it to the bottom descending", () => {
    ...
    expect(sortRows(rows, "auc", "desc").map((r) => r.auc)).toEqual([0.7, 0.5, NaN]);
  });
