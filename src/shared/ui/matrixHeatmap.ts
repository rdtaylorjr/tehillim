...
export interface MatrixHeatmapOptions {
  ...
  /** Psalm numbers in the order `matrix` is indexed by. */
  psalmNumbers: readonly number[];
  matrix: readonly (readonly number[])[];
  /** Psalm numbers in render order, defaulting to `psalmNumbers`. */
  displayOrder?: readonly number[];
  colorScale: (value: number) => string;
  /** Divider positions, in display-order cell counts. */
...
/** Canvas-rendered NxN heatmap, imperative since 22,500 cells are drawn per redraw. */
export class MatrixHeatmap {
  ...
  constructor(options: MatrixHeatmapOptions, classNames: { tooltip: string }) {
    ...
    //: Sized here, since the width attribute is device pixels and would double the layout.
  ...
  private resize(): void {
    ...
    for (const canvas of [this.base, this.overlay]) {
      ...
      canvas.style.width = `${String(this.size)}px`;
      canvas.style.height = `${String(this.size)}px`;
