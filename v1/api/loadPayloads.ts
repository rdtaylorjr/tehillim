...
/** The two small reference payloads ship with the site; the similarity matrices
 * do not. At ~11.5 MB raw they are the same size class as the per-model detail
 * payloads, so they live in the same object-storage tier under the same
 * `/data/detail_` prefix the Worker already routes to R2 — see worker/index.ts. */
