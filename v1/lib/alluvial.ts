/** Bipartite alluvial (Sankey-style) layout: source nodes on one side, target
 * nodes on the other, ribbons between them sized by count. Purpose-built for
 * the genre-alignment view (Gunkel category -> cluster) rather than a general
 * multi-stage Sankey - with exactly two fixed columns there is no
 * crossing-minimization or node-reordering problem to solve, so this
 * hand-rolled version stays a small, pure function instead of pulling in a
 * general-purpose Sankey library for a shape it doesn't need.
 */

export interface AlluvialNode {
  label: string;
  /** Row/column total this node represents (a genre's total psalm count, or a
   * cluster's total size). */
  value: number;
  y0: number;
  y1: number;
}
 ...
 * `height`-px budget shared with `gap`-px breathing room between every pair of
 * nodes that both have a nonzero total - a zero-total node (a genre with no
 * psalms at all in this context) collapses to y0 === y1 and consumes no gap of
 * its own, rather than leaving a visible empty slot. */
function stackNodes(
...
/** Lays out a genre x cluster (or any category x category) alluvial diagram:
 * node height proportional to row/column totals, link (ribbon) width
 * proportional to the shared count, each node's own span partitioned exactly
 * across its links in the order links are visited (source-major for the source
 * side, target-major for the target side) - no gaps or overlaps within a single
 * node's edge. */
