...
describe("the dark-ground palette", () => {
  it("keeps every book hue light enough to read against the panel", () => {
    // The panel sits at --bg-panel #1a1d21 (lightness ~0.12). Every categorical
    // hue has to sit clearly above that rather than below it, which is the whole
    // reason v1's light-mode palette was lifted rather than reused as-is.
    const scale = createBookColorScale();
    for (const book of [1, 2, 3, 4, 5]) {
  ...
  it("keeps within-family genre shades above the panel too", () => {
    // The lightness band was raised from v1's 0.36-0.72 for exactly this.
    for (const shade of hueShades("#5b9bea", 4)) {
      expect(hsl(shade).l).toBeGreaterThan(0.4);
    }
  });
});
