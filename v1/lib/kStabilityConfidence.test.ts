...
describe("describeKStability", () => {
  ...
  // The shipped clustering payload predates this diagnostic entirely, so the
  // absent case is the one that actually reaches production today.
  it("returns null when the field is absent, exactly as for an explicit null", () => {
    expect(describeKStability(2, undefined)).toBeNull();
  });

  it("returns null rather than printing NaN for a non-finite stability", () => {
    expect(describeKStability(2, Number.NaN)).toBeNull();
  });
});
