...
import type { ClusterMethodPayload, GenreAlignment, PsalmCore } from "../model/types";
import {
  buildPoints,
...
describe("buildPoints", () => {
  ...
  it("defaults an unassigned psalm's cluster to -1", () => {
    const method = makeMethod({
      embedding: { x: [0], y: [0], structureCaptured: 1 },
      assignments: {},
    });
    const [point] = buildPoints(psalms([1]), method);
...
describe("degenerateHullCentroid", () => {
  // The fallback used when d3.polygonHull returns null (fewer than 3
  // points) - measured directly against real data: every anisotropy-
  // corrected "whitened" signal (data-driven k in the 6-10 range) puts at
  // least one cluster this small, and d3.polygonCentroid divides by a
  // degenerate polygon's zero signed area, producing NaN for the
  // scatter plot's hull marker. This is the mean-of-points replacement
  // that never divides by an area at all.
  it("is the point itself for a single-member cluster", () => {
    expect(degenerateHullCentroid([[3, 4]])).toEqual([3, 4]);
  });

  it("is the midpoint for a two-member cluster", () => {
    expect(
      degenerateHullCentroid([
        [0, 0],
        [4, 2],
  ...
  it("is the arithmetic mean for more than two points", () => {
    expect(
      degenerateHullCentroid([
        [0, 0],
        [3, 0],
  ...
  it("is the origin for an empty cluster, never NaN", () => {
    expect(degenerateHullCentroid([])).toEqual([0, 0]);
  });
});
