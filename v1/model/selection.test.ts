...
import type { ClusterState, CompareState } from "./selection";
...
describe("initialClusterState", () => {
  ...
  it("opens colored by Gunkel family, since genre recovery is the page's point", () => {
    // Unlike Compare, this page is checking a clustering against the received
    // taxonomy, so the picker shows that ground truth from the start.
...
describe("the two pages' reducers", () => {
  it("keep their own selections apart", () => {
    // They overlap on only selectedPsalm and referenceColorMode, which is why
    // they stay two small reducers rather than one parameterized abstraction.
